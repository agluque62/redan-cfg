		 				...
		 				if (data != 'Configuration not found.'){
							$.each(data.result, function(index, value){
								var item = $('<li data-texto="' + value.idEMPLAZAMIENTO + '"  >' + 
												'<a draggable="true" ondragstart="dragGatewayToSite(event)" ondrop="dropGatewayToSite(event)" ondragover="allowDrop(event)" style="display:block; color:#ffCC00" onclick="CheckingAnyChange(\'GeneralContent\', function(){ShowSite(\'' + value.nameSite + '\',\'' + value.idEMPLAZAMIENTO + '\')})"' + '>' + value.nameSite + '</a>' +
		 				...
		 				if (data != 'Configuration not found.'){
							$.each(data.result, function(index, value){
								...
								var item = $('<li data-texto="' + value.idEMPLAZAMIENTO + '" draggable="true" ondragstart="dragSiteToCfg(event)">' + 
												'<a style="display:block; color:#ffCC00" onclick="CheckingAnyChange(\'GeneralContent\', function(){ShowSite(\'' + value.nameSite + '\',\'' + value.idEMPLAZAMIENTO + '\')})"' + '>' + value.nameSite + '</a>' +
												'<ul class="gtwList" id="site-' + value.idEMPLAZAMIENTO + '" style="display:none"></ul>' + 
											'</li>');
								item.appendTo($(lista));										
							});
