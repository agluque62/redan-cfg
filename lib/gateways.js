	...
	connection.connect(function(err, usrs) {
		if (err) {
			...
			async.waterfall([
				...
				var a = 1;// result now equals 'done'
			});
			function myFirstFunction(callback) {
				var query=connection.query('SELECT name, ipv FROM cgw WHERE idCGW =?',IdGateway ,
					function(err, rows, fields) {
						...
						if (err || rows.length == 0){
							...
							return callback(null, gtw2Change, name);
						}
					}
				);
			}
			function mySecondFunction(arg1, arg2, callback) {
				...
				var query=connection.query('SELECT name, ipv FROM cgw '+ 
					'WHERE EMPLAZAMIENTO_idEMPLAZAMIENTO IN (SELECT idEMPLAZAMIENTO '+ 
					'FROM emplazamiento '+ 	'WHERE cfg_idCFG = (SELECT cfg_idCFG '+ 
					'FROM emplazamiento '+ 	'WHERE idEMPLAZAMIENTO = ? ))', targetGatewaySite ,
					function(err, rows, fields) {
						logging.LoggingDate(query.sql);
						if (err || rows.length == 0){
							return callback(rows.length == 0 ? 'NO_DATA' : err);
