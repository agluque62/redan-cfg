...
exports.changeGateWaySite = function changeGateWaySite(IdGateway, targetGatewaySite, f){
	...
	connection.connect(function(err, usrs) {
		if (err) {
			...
			var query = "";
			query = connection.query('UPDATE cgw SET EMPLAZAMIENTO_idEMPLAZAMIENTO=? WHERE idCGW=?', [targetGatewaySite, IdGateway], function (err, result) {
				logging.LoggingDate(query.sql);
				connection.end();
				f({error: result.affectedRows});
			});
		}
	});
}
							...
							getEmplazamiento(gtw.emplazamiento,idConf,function(result){
								if (result.error === null){
									...
									if (servicioId != null){
										...
										// 	}
										//Vamos a actualizar la id de servicio
										var ultimoServicio = service.name.replace('-SERVICE', '');
										var queryServicio = connection.query('SELECT idSERVICIOS FROM servicios WHERE name=?',ultimoServicio,function(err,result) {
											if ((err != null || rows.length != 0)) {
												connection.end();
												return f({error: 'ER_SEL_SERVICE_NAME'});
											}
											var query = connection.query('INSERT INTO CGW SET servicio=?,EMPLAZAMIENTO_idEMPLAZAMIENTO=?,name=?,dualidad=?,ipv=?,ips=?,nivelconsola=?,puertoconsola=?,nivelIncidencias=?',
														[result[0].idSERVICIOS,gtw.EMPLAZAMIENTO_idEMPLAZAMIENTO,gtw.name,gtw.dualidad,gtw.ipv,gtw.ips,gtw.nivelconsola,gtw.puertoconsola,gtw.nivelIncidencias], function(err, result) {
												if (err){
													logging.loggingError("SQL: " + query.sql + "\nERROR: " + err.message);
													return callback(null, {error: err, data: newGateway});
												}
...
exports.delGateway = function delGateway(req, res, gtw){
	...
	connection.connect(function(err){
		if (err){
			...
			var query = connection.query('DELETE FROM CGW WHERE idCGW=? ' +
										 	'AND ? NOT IN(SELECT idCGW FROM (SELECT idCGW FROM cgw c ' +
											'INNER JOIN cgw_cfg cc on cc.CGW_idCGW=c.idCGW AND cc.Activa=1) AS derived)'
											,[gtw,gtw], function(err, result){
					logging.LoggingDate(query.sql);
					res.json({error: err, data: result.affectedRows});
				});
			connection.end();
		}
	});
...
function updateCgw(connection, gtw, callback){
	...
	var query = connection.query('UPDATE CGW SET ? WHERE idCGW=?',[gtw, gtw.idCGW], function(err, result){
		if (err) return callback(err);
